...
from verl.workers.reward_manager import parallel_compute_score_async
from verl.trainer.ppo.codeact_trainer import CodeActPPOTrainer
...
class RewardManager():
    ...
    def __init__(self, tokenizer, num_examine, config, compute_score=None) -> None:
        ...
        # from verl.utils.reward_score.openhands_swebench import load_swebench_dataset
    ...
    def verify(self, data):
        ...
        # print(f"(DL Debug in RewardManager): length: {len(response_str)} {len(ground_truth)} {len(extra_info)} Extra info: {extra_info}")
        data_source = [self.data_source] * len(response_str)
        # print(f"(DL Debug in RewardManager): response str : {response_str}")
        # print(f"(DL Debug in RewardManager): ground truth : {ground_truth}")
        # print(f"(DL Debug in RewardManager): data source : {data_source}")
        # DL: Async Logic adapted from PRIME
        try:
        ...
        # print(score)
        # assert False
        # score = [0.5 for _ in range(len(response_str))]
...
def main_task(config, compute_score=None):
    ...
    from verl.trainer.ppo.codeact_trainer import ResourcePoolManager, Role
    ...
    trainer = CodeActPPOTrainer(config=config,
                            tokenizer=tokenizer,
