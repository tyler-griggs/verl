...
def convert_right_padding_to_left(tokenizer, input_ids, attention_mask):
    """
    ...
    # Pad input ids to tokenizer pad id
    left_padded_input_ids = torch.empty_like(input_ids)
    left_padded_input_ids.fill_(tokenizer.pad_token_id)
...
class CodeActAgentGroup:
    ...
    def _convert_results_to_dataproto(self) -> DataProto:
        ...
        base_dir = "/mnt/user_storage/logs/results/16/step" + str(self.global_step)
        ...
        prompt_input_ids, prompt_attention_mask = convert_right_padding_to_left(self.tokenizer, prompt_input_ids, prompt_attention_mask)
        ...
        print(f"input_ids shape: {input_ids.shape}, response_ids shape: {response_ids.shape}, attention_mask shape {attention_mask.shape}")
