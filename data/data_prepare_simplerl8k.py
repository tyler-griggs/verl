...
qwq_prompt="You are a helpful and harmless assistant. You are Qwen developed by Alibaba. You should think step-by-step."
...
if __name__=='__main__':
    ...
    parser.add_argument('--input',type=str, default='/home/ubuntu/tgriggs/data/math_level3to5_data_processed_with_qwen_prompt.json')
    parser.add_argument('--output', type=str, default='dataset/prime')
    args=parser.parse_args()

    input_dataset = load_json_dataset(args.input)
    
    output_dataset=[]
    cur_dataset = input_dataset
    idx = -1
    for data_entry in tqdm(cur_dataset):
        cur_data = {
            "data_source": "simplerl",
            "prompt": [{
                "role": "system",
    ...
    output_dataset.to_parquet(os.path.join(args.output, 'train.parquet'))
